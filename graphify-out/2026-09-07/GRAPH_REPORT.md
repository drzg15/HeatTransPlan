...
## Corpus Check
- 151 files · ~282,432 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1433 nodes · 2806 edges · 93 communities (55 shown, 22 thin omitted)
...
- `validate_cop_formula()` --uses--> `CopFormulaValidateRequest`  [INFERRED]
  backend/app/routers/analysis.py → backend/app/models/analysis.py
- `validate_cop_formula()` --uses--> `CopFormulaValidateResult`  [INFERRED]
  backend/app/routers/analysis.py → backend/app/models/analysis.py
- `run_hpi_optimization()` --uses--> `HPIOptimizationRequest`  [INFERRED]
  backend/app/services/optimization_service.py → backend/app/models/analysis.py
...
### Community 2 - "Pinch"
Cohesion: 0.08
Nodes (5): PinchExport, Pinch, PinchPlot, Streams, Exception
...
Cohesion: 0.08
Nodes (39): AST, HPIOptimizationDiagnostics, Why the optimization found little or nothing. Filled on every run, so an empty…, Check a user COP formula and evaluate it at one sample point. Called as the…, validate_cop_formula(), _diagnostic_messages(), _formula_alternative(), _load_model_and_data() (+31 more)

### Community 5 - "react"
Cohesion: 0.10
Nodes (29): getExample(), App(), DataCollectionPage, PotentialAnalysisPage, ScenarioComparison(), ActionBar(), BASE_OPTIONS, Props (+21 more)

### Community 7 - "trigger"
Cohesion: 0.13
Nodes (4): Qn, remove(), Sn, trigger()

### Community 8 - "models/analysis.py"
Cohesion: 0.13
Nodes (25): CopFormulaSpec, CopFormulaValidateRequest, CopFormulaValidateResult, EnergyDemand, HeatPumpEntry, HPIOptimizationResult, OptimizedIntegrationPoint, PinchResult (+17 more)
...
### Community 12 - "analysis_service.py"
Cohesion: 0.15
Nodes (18): HPIntegrationResult, HPIRequest, HPIResult, Output of HPI analysis — one entry per HP type evaluated., Input to the HPI endpoint., Result for a single heat pump type., HeatPumpOutOfRange, A named heat pump technology was asked for a duty outside its rating. Raised… (+10 more)
...
### Community 19 - "models/__init__.py"
Cohesion: 0.14
...
Cohesion: 0.13
Nodes (7): createDivIcon(), GroupCoords, StreamCirclesOverlay(), streamColor(), TILE_URLS, leaflet, react-leaflet
...
Cohesion: 0.17
Nodes (14): Props, StreamCirclesOverlayProps, GroupCoords, ProcessGroupList(), Props, SubprocessCard(), defaultState, ProjectStore (+6 more)

### Community 31 - "routers/analysis.py"
Cohesion: 0.14
Nodes (19): HPIOptimizationRequest, Input to the status quo comparison endpoint., Comparison of current vs pinch-optimal demands., Input to the HPI optimization endpoint., StatusQuoRequest, StatusQuoResult, generate_report(), get_map_preview() (+11 more)
...
### Community 38 - "SubprocessCard.tsx"
Cohesion: 0.19
...
### Community 42 - "PinchMain"
Cohesion: 0.13
Nodes (17): CompositeDiagramData, PinchRequest, PinchStream, A single stream for pinch analysis input., Input to the pinch analysis endpoint., H-T data for hot and cold composite curves., PinchMain, pinch_analysis() (+9 more)

### Community 44 - "map_service.py"
Cohesion: 0.15
Nodes (16): generate_process_map_b64(), Map rendering service. Generates images with process boxes and stream circles…, Generate a base64 encoded PNG image of the map with processes and streams…, haversine_distance(), lonlat_to_tile_xy(), Geo utilities — extracted from data_collection.py. Web Mercator projection +…, Convert lon/lat to tile x/y at given zoom level., Convert tile x/y to lon/lat at given zoom level. (+8 more)
...
### Community 57 - "report_service.py"
Cohesion: 0.22
Nodes (13): Stream data for report generation., ReportStream, _fmt(), generate_html_report(), _map_b64_cached(), Any, Report generation service — produces self-contained HTML report. Includes…, Format a numeric value, return '' if None. (+5 more)

### Community 59 - "scripts"
Cohesion: 0.29
Nodes (7): scripts, build, dev, lint, prepare, preview, test

### Community 60 - "routers/streams.py"
Cohesion: 0.29
...
- **Why does `react-i18next` connect `react` to `StreamDataTable.tsx`, `HPIChart.tsx`, `SubprocessCard.tsx`, `types/analysis.ts`, `PotentialAnalysisPage.tsx`, `package.json`, `useUIStore`, `ProcessNode`?**
